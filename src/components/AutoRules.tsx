  ...
  return (
    <div className="flex items-center justify-between py-2 px-3 bg-white rounded-xl border border-slate-100 group">
      <div className="flex-1 min-w-0">
        <div className="text-[11px] text-slate-600 font-medium leading-tight">{threshold.label}</div>
      </div>
      <div className="flex items-center gap-1 flex-shrink-0 ml-3">
        <button onClick={dec}
          className="w-6 h-6 rounded-lg flex items-center justify-center text-[11px] font-bold transition-all hover:scale-110 active:scale-95"
        ...
        <div className="min-w-[48px] flex items-center justify-center gap-0.5 font-mono">
          <span className="text-[13px] font-black" style={{ color: threshold.direction === '+' ? '#059669' : '#DC2626' }}>
  ...
  return (
    ...
    >
      ...
      <div className="px-4 pt-4 pb-3 flex items-start gap-3">
        {/* Ordre */}
        <div className="w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-bold bg-slate-100 text-slate-400 flex-shrink-0 mt-0.5">
          {rule.order}
        </div>
        {/* Icône */}
        ...
        {/* Titre + badges */}
        <div className="flex-1 min-w-0">
          <div className="text-[13px] font-bold text-slate-900 leading-tight mb-1">{rule.name}</div>
        ...
        <div className="flex flex-col items-end gap-2 flex-shrink-0">
          <span className="flex items-center gap-1 text-[9px] font-semibold px-2 py-1 rounded-lg"
      ...
      <div className="px-4 pb-3 flex-1">
        <p className="text-[11px] text-slate-500 leading-relaxed mb-3">{rule.description}</p>

        {/* Trigger + Valeur ajoutée */}
        <div className="space-y-1.5 mb-3">
          <div className="flex items-start gap-2">
            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider min-w-[70px] mt-0.5">Déclencheur</span>
            <span className="text-[11px] text-slate-600 font-medium leading-tight">{rule.trigger}</span>
          </div>
          <div className="flex items-start gap-2">
            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider min-w-[70px] mt-0.5">Valeur ajoutée</span>
            <span className="text-[11px] font-semibold leading-tight" style={{ color: rule.color }}>{rule.added_value}</span>
          </div>
        </div>
        ...
        {rule.thresholds.length === 0 && (
          <div className="text-[10px] text-slate-400 italic">Aucun seuil — restriction on/off uniquement</div>
        )}
      </div>
      ...
      <button
        ...
        className="flex items-center justify-between w-full px-4 py-2.5 border-t border-slate-100 bg-slate-50/60 hover:bg-slate-100/70 transition-all text-[10px] font-semibold text-slate-500"
      >
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            <div className="p-4 grid grid-cols-2 gap-4 bg-slate-50/50 border-t border-slate-100">
              {/* Conditions */}
              <div>
                <div className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-2">Conditions SI</div>
                <div className="space-y-1.5">
                  {rule.conditions.map((cond, i) => (
                    <div key={i} className="flex items-center gap-2 bg-white rounded-xl px-3 py-1.5 border border-slate-100">
                      {cond.locked && (
                        <Lock className="w-2.5 h-2.5 text-slate-300 flex-shrink-0" title="Valeur figée — configurable dans les paramètres globaux" />
                      )}
                      <span className="text-[10px] text-slate-500 font-medium min-w-[70px] truncate">{cond.label}</span>
                      <span className="text-[9px] font-bold text-slate-300 mx-0.5">{cond.operator}</span>
                      <span className={`text-[10px] font-bold flex-1 truncate ${cond.locked ? 'text-slate-400' : ''}`}
              ...
              {/* Actions */}
              <div>
                <div className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-2">Actions ALORS</div>
                <div className="space-y-1.5">
                  {rule.actions.map((action, i) => (
                    <div key={i} className="flex items-center gap-2 bg-white rounded-xl px-3 py-1.5 border border-slate-100">
                      ...
                      <span className="text-[10px] font-semibold" style={{ color: action.color }}>{action.label}</span>
                    </div>
                  ))}
                </div>
                {/* Impact */}
                <div className="mt-2 p-2.5 rounded-xl flex items-start gap-1.5" style={{ background: rule.bg }}>
                  ...
                  <span className="text-[10px] leading-relaxed" style={{ color: rule.color }}>{rule.impact}</span>
                </div>
              </div>
            </div>
            {/* Warning */}
            {rule.warning && (
              <div className="px-4 pb-4 bg-slate-50/50">
                <div className="p-3 bg-red-50 rounded-xl flex items-start gap-2 border border-red-100">
                  ...
                  <span className="text-[11px] text-red-600 font-medium leading-relaxed">{rule.warning}</span>
                </div>
              </div>
            )}
          </motion.div>
